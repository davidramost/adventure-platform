            ...
            <main className="flex-1 bg-gradient-to-b from-gray-900 to-gray-800 py-16">
                <div className="max-w-4xl mx-auto px-4">
                        ...
                        <div className="flex flex-col md:flex-row items-start gap-8">
                            ...
                            <div className="flex-1 w-full">
                                <div className="mb-8 pb-6 border-b border-gray-700">
                                    <p className="text-gray-400 text-xs mb-4 flex items-center gap-2 tracking-wider font-semibold">
                                        Identidad de tu cuenta
                                        <div className="relative group">
                                            <svg
                                                ...
                                                className="w-4 h-4 cursor-help text-gray-500 hover:text-gray-300 transition-colors"
                                            >
                                                <circle cx="12" cy="12" r="10" />
                                                <path d="M12 16v-4" />
                                                <path d="M12 8h.01" />
                                            </svg>
                                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-gray-700 text-gray-100 text-xs rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                                                No se pueden editar para mantener la seguridad de tu cuenta
                                            </div>
                                        </div>
                                    </p>
                                ...
                                <div className="mb-6">
                                    <div className="flex items-center justify-between mb-3">
                                        <label className="block text-white text-sm font-medium">Datos Personales (para
                                            envíos)</label>
                                    ...
                                    {isEditingPersonalInfo ? (
                                        ...
                                        <div
                                            className="bg-gray-700/50 border border-gray-600 rounded-lg p-3 space-y-2 text-sm">
                                            <p className="text-gray-300">
                                                <span className="text-gray-400">Nombre:</span> {usuario.nombre || '—'}
                                            </p>
                                            <p className="text-gray-300">
                                                <span
                                                    className="text-gray-400">Apellido:</span> {usuario.apellido || '—'}
                                            </p>
                                            <p className="text-gray-300">
                                            ...
                                            <p className="text-gray-300">
                                                <span
                                                    className="text-gray-400">Facturación:</span> {usuario.fact_domicilio || '—'}
                                            </p>
                                        </div>
                                    )}
                                </div>
                    ...
                    <div className="mb-12">
                        ...
                        >
                            <h2 className="text-2xl font-bold text-white">Mis Pedidos
                                ({pedidosLoading ? '...' : pedidos.length})</h2>
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="24"
                                height="24"
                        ...
                        {pedidosOpen && (
                            pedidosLoading ? (
                                ...
                                <div
                                    className="bg-gray-800/50 backdrop-blur border border-gray-700/50 rounded-2xl p-12 text-center">
                                ...
                                <div className="space-y-4">
                                    {pedidos.map(pedido => (
                                        <div key={pedido.id_pedido}
                                            className="bg-gray-800/50 backdrop-blur border border-gray-700/50 rounded-xl p-6">
                                            <div
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-gray-400 text-sm">{new Date(pedido.fecha).toLocaleDateString('es-ES', {
                                                        day: '2-digit',
                                                        month: 'long',
                                                        year: 'numeric'
                                                    })}</p>
                                                ...
                                                <div className="flex items-center gap-4">
                                                    <span
                                                        className="px-3 py-1 bg-gray-700 rounded-full text-xs text-gray-300">{pedido.tipo_envio}</span>
                                                    <span
                                                        className="px-3 py-1 bg-gray-700 rounded-full text-xs text-gray-300">{pedido.metodo_pago}</span>
                                                ...
                                                {pedido.lineas.map(linea => (
                                                    <div key={linea.id_producto} className="flex items-center gap-4">
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            ...
                                                            <p className="text-gray-400 text-xs">x{linea.cantidad} · {linea.precio_unitario.toFixed(2)} €
                                                                / ud</p>
                                                        </div>
                                            ...
                                            {pedido.gasto_envio > 0 && (
                                                <div
                                                    className="border-t border-gray-700 mt-4 pt-3 flex justify-between text-sm">
                                                    <span className="text-gray-400">Gastos de envío</span>
                                                    <span
                                                        className="text-gray-300">{pedido.gasto_envio.toFixed(2)} €</span>
                                                </div>
                                            )}
                                        </div>
                                    ))}
                    ...
                    <div>
                        <h2 className="text-2xl font-bold text-white mb-6">Mis Rutas Favoritas
                            ({authLoading ? '...' : userFavorites.length})</h2>
                        ...
                        {authLoading ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {userFavorites.map(ruta => (
                                    <div key={ruta.id_ruta} onClick={() => navigate(`/ruta/${ruta.id_ruta}`)}
                                        className="bg-gray-800/50 backdrop-blur border border-gray-700/50 rounded-xl overflow-hidden hover:border-primary-light/50 transition-colors group cursor-pointer">
                                        <Image
                                            src={ruta.imagen_url}
